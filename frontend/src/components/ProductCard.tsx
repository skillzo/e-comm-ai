...
export default function ProductCard({ product }: ProductCardProps) {
  const cardContent = (
    <>
      ...
      <div className="mt-2 font-bold text-lg">{formatNaira(product.price)}</div>
    </>
  );
...
export function RelatedProductCard({ product }: ProductCardProps) {
  const cardContent = (
    <div>
      <div className="relative aspect-4/4 bg-gray-100 mb-4 overflow-hidden rounded-sm">
        <img
          alt={product.name}
          className="w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-500"
          src={product.image || ""}
